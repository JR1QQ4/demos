 ...
 * （3）全局对象限制
 * （4）通信联系
 * （5）脚本限制,不能执行alert()方法和confirm()方法
 * （6）文件限制,无法读取本地文件，即不能打开本机的文件系统（file://），它所加载的脚本，必须来自网络。
 * 
 * 主线程
 * 
 ...
 *       doSomething(event.data); // 事件对象的data属性可以获取 Worker 发来的数据
 *     }
...
// 必须指定<script>标签的type属性是一个浏览器不认识的值，如 app/worker。
// <!DOCTYPE html>
//   <body>
...
// // 新建了10个 Worker 线程，并且依次向这10个 Worker 发送消息，告知了计算的起点和终点
